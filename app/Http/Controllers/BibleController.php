...
class BibleController extends Controller
{
    ...
    public function createNextLink(
        ...
        if ($current_book === 66 && $current_chapter === 22) {
            return "";
        }
    ...
    public function createPrevLink(
        ...
        if ($current_book === 1 && $current_chapter === 1) {
            return "";
        }
    ...
    public function createVerseLink(
        string $current_trans,
        int $verse_id,
    ): string {
    ...
    public function index(
        ...
        $text = DB::table($translation)->where($args)->get();
        ...
        $next_link = $this->createNextLink(
            $translation,
            $book_num,
            $chapter_num,
        );
        $prev_link = $this->createPrevLink(
            $translation,
            $book_num,
            $chapter_num,
        );
    ...
    public function single(
        ...
        $next_verse_link = $this->createVerseLink($translation, $next_id);

        $prev_verse_link = $this->createVerseLink($translation, --$prev_id);
        ...
        $seo_title =
            "$single_title $book_title $chapter_num:$verse_display" .
            " — " .
            config("app.fullname");
        $seo_description =
            "$single_title $book_title $chapter_num:$verse_display " .
    ...
    function randomVerse(Request $request)
    {
        ...
        $seo_title =
            "Случайный отрывок из Библии" . " | " . config("app.fullname");
        $seo_description =
            "Генератор случайных отрывков из Библии. Бесполезная, но забавная функция. " .
            strtoupper($translation) .
            ". " .
            config("app.desc");
    ...
    function home(Request $request)
    {
        ...
        $seo_title = config("app.fullname");
        $seo_description = config("app.desc");
        $data = [
            "user_id" => auth()->id() ?? 0,
            "comments" => $comments,
